...
#   https://github.com/ainrichman/Peppa-Facial-Landmark-PyTorch/blob/master/tracker.py
#
...
class LKTracker:
    ...
    def delta_fn(self, prev_points, new_detected, lk_tracked):
        ...
        temp[dist_detect_eyes >= thres] = detect_eyes[dist_detect_eyes >= thres] * weight2 \
                                        + lk_eyes[dist_detect_eyes >= thres] * (1 - weight2)
        temp[dist_detect_eyes < thres] = lk_eyes[dist_detect_eyes < thres] * weight1 \
                                       + detect_eyes[dist_detect_eyes <= thres] * (1 - weight1)
        result[eye_indices] = temp

        thres = 10
        temp = result[rest_indices]
        temp[dist_detect_rest < thres] = lk_rest[dist_detect_rest < thres] * weight1 \
                                       + detect_rest[dist_detect_rest < thres] * (1 - weight1)
        temp[dist_detect_rest >= thres] = detect_rest[dist_detect_rest >= thres] * weight2 \
                                        + lk_rest[dist_detect_rest >= thres] * (1 - weight2)
